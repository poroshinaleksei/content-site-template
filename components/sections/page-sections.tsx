...
async function renderSection(section: PageSection) {
  ...
  switch (section.type) {
    ...
    case "articles":
      return <ArticlesSection key={section.id} section={section} />;
    case "cta":
...
export async function PageSections({ sections }: PageSectionsProps) {
  return <>{await Promise.all(sections.map(renderSection))}</>;
}
